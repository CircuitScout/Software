...
socket.on("probe_candidates", (candidates) => {
    ...
    candidatesLabel1.innerHTML = "Probe 1: " + layoutPads[candidates["left"]]["net"];
    candidatesLabel2.innerHTML = "Probe 2: " + layoutPads[candidates["right"]]["net"];
});
...
function updateProbeOffsets() {
    ...
    socket.emit("update_probe_offsets", probeOffsets);
}


function updateSelectedDevice(e) {
    let selectedPort = e.target.value;
    socket.emit("selected_serial_device", selectedPort);
}
