...
def get_pads(layout: Board, origin: List):
    ...
    pads = {
        ...
        } for f, gc in zip(footprints, grav_centers) for p in f.pads
    }
...
def local_to_board_coordinates(pad: dict, dimensions: dict) -> Tuple[float, float, float]:
    """Converts local coordinates to board coordinates"""

    x_offset = (dimensions["height"] if dimensions["orientation"] == 90 else 0) + (dimensions["width"] if dimensions["orientation"] == 180 else 0)
    y_offset = (dimensions["height"] if dimensions["orientation"] == 180 else 0) + (dimensions["height"] if dimensions["orientation"] == 270 else 0)
    ...
    board_angle = dimensions["orientation"] * math.pi / 180
    ...
    x = rotated_pad_x + x_offset
    y = rotated_pad_y + y_offset
...
def select_nets():
    ...
    if (
        ...
        for pad in pads_data[user_id].values():
            if not pad["is_allowed"]:
                continue

            candidate_pad_x, candidate_pad_y, relative_angle = local_to_board_coordinates(pad, layout_dimensions)
        ...
        if len(first_coordinates) > 0 and len(second_coordinates) > 0:
            ...
            candidates_data[user_id] = {
                "left": left_coordinates[0],
                "right": right_coordinates[0],
            }
